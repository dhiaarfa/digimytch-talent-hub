...
export function ApiKeysForm({ isProPlan }: { isProPlan: boolean }) {
  ...
  const handleUpdateKey = (service: ServiceName) => {
    ...
    if (!keyValue?.trim()) {
      toast.error(digimytch ? "Saisissez une clé API" : "Please enter an API key")
      return
    }
  ...
  const visibleProviders = getProvidersArray().filter((p) => {
    if (p.unstable) return false
    if (digimytch) return p.id === "openrouter"
    return true
  })
    ...
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-xl bg-gradient-to-br from-white/50 to-white/30 border border-white/40 shadow-xl backdrop-blur-sm">
        <Label className="text-base font-semibold bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent">
          {digimytch ? "Modèle IA par défaut" : "Default AI Model"}
        </Label>
        <p className="text-sm text-muted-foreground mt-2 mb-3">
          {digimytch
            ? "Utilisé pour le matching, les lettres de motivation, l'entretien simulé et les assistants CV."
        ...
        <ModelSelector
          ...
          placeholder={digimytch ? "Choisir un modèle" : "Select an AI model"}
        />
      </div>
      ...
      <div className="p-5 rounded-xl bg-gradient-to-br from-white/50 to-white/30 border border-white/40 shadow-xl backdrop-blur-sm">
        <Label className="text-base font-semibold bg-gradient-to-r from-teal-600 to-cyan-600 bg-clip-text text-transparent">
          {digimytch ? "Clé OpenRouter (optionnelle)" : "API Keys"}
        </Label>
        <div className="mt-2 mb-4 space-y-2">
          <p className="text-sm text-muted-foreground">
            {digimytch
              ? "Si OPENROUTER_API_KEY est déjà dans le fichier .env du projet, vous n'avez rien à saisir ici. Sinon, ajoutez votre clé — elle reste dans votre navigateur."
