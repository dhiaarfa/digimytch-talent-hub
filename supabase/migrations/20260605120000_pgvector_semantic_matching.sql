...
ALTER TABLE public.jobs
  ADD COLUMN embedding storage.vector(1536);
...
CREATE INDEX idx_jobs_embedding_hnsw
  ON public.jobs
  USING hnsw (embedding storage.vector_cosine_ops)
...
CREATE OR REPLACE FUNCTION public.match_jobs_semantic(
  p_user_id uuid,
  p_resume_embedding storage.vector(1536),
...
GRANT EXECUTE ON FUNCTION public.match_jobs_semantic(uuid, storage.vector, float, int) TO authenticated;
